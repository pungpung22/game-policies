...
| Firebase Remote Config Android | 예 | 예 | 앱 업데이트 버전·최소 지원 버전·안내 문구 전달 |
| Firebase Crashlytics + NDK Android | 예 | 예 | Java/Godot 네이티브 충돌 및 비치명 오류 진단 |
| Firebase iOS 앱 구성 | 등록만 | 현재 아니요 | Console/config 등록 완료, iOS SDK 브리지 전 출시 차단 |
| Google Analytics | 아니요 | 해당 없음 | Firebase 프로젝트에서 사용 안 함 |
...
## Firebase가 처리할 수 있는 범주

- Remote Config: 앱 범위 Firebase 설치 식별자와 업데이트 구성 요청 정보
- Crashlytics: 충돌 보고서, 스택 트레이스, 앱·OS 버전, 기기 모델, 프로세스 상태,
  발생 시각, 앱 버전·플랫폼·엔진 범위 같은 제한된 custom key
- 계정, 이메일, 로컬 저장 데이터, 광고 ID, 정밀 위치는 Crashlytics custom key에
  의도적으로 넣지 않음
- 목적: 앱 업데이트 안내, 안정성 문제 진단, 앱 신뢰성 개선
...
6. iOS Firebase SDK/브리지를 추가하면 최종 IPA와 App Privacy를 다시 검토한다.
7. mediation, 분석, 추가 진단 SDK, IAP provider를 추가하면 이 목록과 모든 스토어 신고를 갱신한다.
